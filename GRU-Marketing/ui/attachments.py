...
def uploaded_csv_rows(
    attachments: list[DecodedAttachment],
) -> dict[str, list[dict[str, str]]]:
    """Parse uploaded CSVs for deterministic tools that cannot read Gemini parts."""
    parsed: dict[str, list[dict[str, str]]] = {}
    for item in attachments:
        if Path(item.filename).suffix.lower() != ".csv":
            continue
        text = item.data.decode("utf-8-sig", errors="replace")
        parsed[item.filename] = [
